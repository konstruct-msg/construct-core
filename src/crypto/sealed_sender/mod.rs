...
pub fn seal_sender_cert(
    ...
    let recipient_pub = PublicKey::from(as_key32(recipient_identity_key, "recipient identity key")?);
...
pub fn verify_sender_cert(
    ...
    let payload =
        build_cert_sign_payload(user_id, domain, identity_key, device_id, issued_at, expires_at);
...
mod tests {
    ...
    fn unseal_rejects_truncated_box() {
        ...
        assert!(unseal_sender_cert(&[0u8; EPH_LEN + NONCE_LEN + TAG_LEN - 1], &recipient_priv).is_err());
    }
    ...
    fn verify_cert_round_trip_and_rejections() {
        ...
        let ik = [0xCD; 32];
        let payload =
            build_cert_sign_payload("user-1", "example.org", &ik, "dev-1", 100, 200);
        let sig = sk.sign(&payload);

        assert!(verify_sender_cert(
            "user-1", "example.org", &ik, "dev-1", 100, 200,
            &sig.to_bytes(), vk.as_bytes()
        ));
        // Any field change breaks the signature.
        assert!(!verify_sender_cert(
            "user-2", "example.org", &ik, "dev-1", 100, 200,
            &sig.to_bytes(), vk.as_bytes()
        ));
        assert!(!verify_sender_cert(
            "user-1", "example.org", &ik, "dev-1", 100, 201,
            &sig.to_bytes(), vk.as_bytes()
        ));
